...
class Mikrotik_rules_attr():
    def __init__(self,chain,addr,script_name,v6,list,interface, stage, portrange):
        self.chain = chain
        self.addr = addr
    ...
    def show(self):
        ...
        print(f"portrange: {str(self.portrange[0])}-{self.portrange[1]}")
    ...
    def Stages(self):
        ...
        i = 0
        while i < self.stage:
            port = random.randint(self.portrange[0],self.portrange[1])
            if port in ports:
                if i > 0:
                    i -= 1
                else:
